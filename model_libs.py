...
def DeConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, lr_mult=1,
    deconv_prefix='', deconv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    **bn_params):
  # parameters for convolution layer with batchnorm.
  eps = bn_params.get('eps', 0.001)
  ...
  net[deconv_name] = L.Deconvolution(net[from_layer], 
      convolution_param=dict(num_output=num_output, 
                    kernel_size=kernel_size, 
                    stride=stride, 
                    pad = pad,
                    bias_term=False),
      param={"lr_mult": lr_mult, "decay_mult": 1})
  if dilation > 1:
    net.update(deconv_name, {'dilation': dilation})
  if use_bn:
    bn_name = '{}{}{}'.format(bn_prefix, out_layer, bn_postfix)
    print("bn_name: ", bn_name, bn_prefix, out_layer, bn_postfix)
    net[bn_name] = L.BatchNorm(net[deconv_name], in_place=True, **bn_kwargs)
    if use_scale:
      sb_name = '{}{}{}'.format(scale_prefix, out_layer, scale_postfix)
      net[sb_name] = L.Scale(net[bn_name], in_place=True, **sb_kwargs)
    else:
...
def DeconvConnect(net, block_name, from_upsam, from_bottom, out_channels, k=4, p=1, s=2,has_connect=True):
  deconv_prefix = 'up{}'.format(block_name)
  deconv_postfix = ""
  lateral_prefix = 'd{}'.format(block_name)
  lateral_postfix = ''
  bn_prefix = ""
  bn_postfix = '_bn' # .format(block_name)
  scale_prefix = ""
  scale_postfix = '_scale' # .format(block_name)
  use_scale = True

  up_name = deconv_prefix
  DeConvBNLayer(net, from_upsam, up_name, use_bn=True, use_relu=False,
            num_output=out_channels, kernel_size=k, pad=p, stride=s, dilation=1, lr_mult=1,
            deconv_prefix='', deconv_postfix=deconv_postfix,bn_prefix=bn_prefix, bn_postfix=bn_postfix, 
            scale_prefix=scale_prefix, scale_postfix=scale_postfix)

  sum_name = '{}_{}'.format(from_bottom, block_name)
  net[sum_name] = L.Eltwise(net[up_name], net[from_bottom])

  if has_connect:
    out_name = 'd{}'.format(block_name)
    ConvBNLayer(net, sum_name, out_name, use_bn=True, use_relu=False,
        num_output=out_channels, kernel_size=1, pad=0, stride=1, use_scale=use_scale,
        conv_prefix='', conv_postfix=lateral_postfix,bn_prefix=bn_prefix, bn_postfix=bn_postfix,
        scale_prefix=scale_prefix, scale_postfix=scale_postfix)
    return out_name
  else:
    return sum_name
...
def CreatePythonInputLayer(layer_params={}):
    return L.Python(module = 'caffe.pylayers.pyannotatedlayer', 
                    layer = 'PyAnnotatedDataLayer', 
                    ntop = 2, param_str=str(layer_params))

def CreatePythonInputSegLayer(layer_params={}):
    return L.Python(module = 'caffe.pylayers.pyannotatedmasklayer', 
                    layer = 'PyAnnotatedMaskLayer', 
                    ntop = 3, param_str=str(layer_params))

def CreatePySegEvalLayer(bottom0, bottom1, layer_params={}):
    return L.Python(bottom0, bottom1, 
                    module = 'caffe.pylayers.pysegevalayer', 
                    layer = 'PySegEvalLayer', 
                    ntop = 1, param_str=str(layer_params))
...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    for i in range(0, num):
        ...
        if max_size:
            num_priors_per_location = len(aspect_ratio) * len(min_size) # (2 + len(aspect_ratio)) * len(min_size)
        else:
            num_priors_per_location = len(aspect_ratio) * len(min_size) # (1 + len(aspect_ratio)) * len(min_size)
        if flip:
        ...
        print("num_loc_output: ", num_loc_output, " num_conf_output: ", num_conf_output)
...
def CreateMultiBoxHead_box(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
    ...
    for i in range(0, num):
        ...
        num_priors_per_location = len(aspect_ratio) * (len(min_size) + len(max_size))
        step = []
        if len(steps) > i:
            step = steps[i]
        """
        ...
        name = "{}_mbox_loc{}".format(from_layer, loc_postfix)
        num_loc_output = num_priors_per_location * 4;
        print("num_loc_output: ", num_loc_output)
        if not share_location:
            num_loc_output *= num_classes
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_loc_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        ...
        name = "{}_mbox_conf{}".format(from_layer, conf_postfix)
        num_conf_output = num_priors_per_location * num_classes;
        print("num_conf_output: ", num_conf_output)
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_conf_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
