...
# ('Final Segmentaiton: miou: ', 0.5197170370984521, ' recall: ', 0.4500462641378257, ' precision: ', 0.6629712642137454)
def main(args):
    '''main '''
    ...
    dst_path = '/home/yaok/software/caffe_ssd/result/box/mask_96/inter'
    ...
    for img_name in img_list:
        ...
        h, w, _ = img.shape
        det_mask_gray = det_mask[:,:,0].astype(np.int64)
        inter = cv2.bitwise_and(det_mask_gray, seg_mask)

        resize_mask = cv2.resize(inter, (h, w), interpolation=cv2.INTER_NEAREST)
        ...
        miou_seg, rec_seg, prec_seg = seg_scores(gt_mask, inter, n_cls)
