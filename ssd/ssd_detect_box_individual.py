...
class CaffeSegmentation:
    def __init__(self, gpu_id, model_def, model_weights, image_resize):
        caffe.set_device(gpu_id)
        caffe.set_mode_gpu()
        self.img_resize = image_resize
        self.net = caffe.Net(model_def, model_weights, caffe.TEST)
        self.mean = np.array([83.617, 110.63, 118.362])

    def detect(self, img_file, conf_thresh=0.6):
        caffe.set_device(0)
        caffe.set_mode_gpu()
        pic_name = img_file# .split('/')[-1]
        ...
        self.net.forward()
        segmask = self.net.blobs['argmax'].data
        return segmask[0][0]
...
def seg_union(det, det_result, seg_result, n_cls):
...
def seg_inter(det, det_result, seg_result, n_cls):
    ...
    inter = cv2.bitwise_and(det_mask_gray, seg_result)
    return inter
...
# union: ('Final Segmentaiton: miou: ', 0.6698136325793186, ' recall: ', 0.9244987309115551, ' precision: ', 0.7270551908273757) 200ms
# inter: ('Final Segmentaiton: miou: ', 0.790487883179413, ' recall: ', 0.8639350175570707, ' precision: ', 0.9099915861471161) 180ms
# inter threshold: 0.01, ('Final Segmentaiton: miou: ', 0.81303010460939, ' recall: ', 0.8390352963164386, ' precision: ', 0.9646245536596515)
# inter threshold: 0.002, ('Final Segmentaiton: miou: ', 0.874334083185864, ' recall: ', 0.9046873303396936, ' precision: ', 0.9635670052007232)
# inter threshold: 0.001, ('Final Segmentaiton: miou: ', 0.8754339166292775, ' recall: ', 0.9057886132085218, ' precision: ', 0.9637034570364684)
# inter threshold: 0.005, ('Final Segmentaiton: miou: ', 0.8629889674376402, ' recall: ', 0.8928297956111072, ' precision: ', 0.9632771410416101)
# inter threshold: 0.008, ('Final Segmentaiton: miou: ', 0.8384871110054571, ' recall: ', 0.8661477585551899, ' precision: ', 0.9644898829576657)
# inter threshold: 0.0005, ('Final Segmentaiton: miou: ', 0.8746533004168285, ' recall: ', 0.9063787233571432, ' precision: ', 0.9621316115629097)

def main(args):
    '''main '''
    detection = CaffeDetection(args.gpu_id,
                               args.det_model_def, args.det_model_weights,
                               args.det_image_resize, args.labelmap_file)

    segmentation = CaffeSegmentation(args.gpu_id,
                               args.seg_model_def, args.seg_model_weights,
                               args.seg_image_resize)
    img_dir = args.image_file
    img_path = '/media/data/seg_dataset/fbox/JPEGImages'
    dst_path = '/home/yaok/software/caffe_ssd/result/box/indivadual/threshold_8e3'
    ...
    for img_name in img_list:
        ...
        seg_mask = segmentation.detect(full_name)
        ...
        # union = seg_union(detection, result, seg_mask, n_cls)
        # inter = seg_inter(detection, result, seg_mask, n_cls)
        inter_threshold = seg_inter_threshold(detection, result, seg_mask, n_cls, gamma)

        miou_seg, rec_seg, prec_seg = seg_scores(gt_mask, inter_threshold, n_cls)
        ...
        resize_mask = cv2.resize(inter_threshold, (h, w), interpolation=cv2.INTER_NEAREST)
...
def parse_args():
    ...
    parser.add_argument('--det_model_def',
                        default='/home/yaok/software/caffe_ssd/models/VGGNet/fbox/SSD_300x300/deploy.prototxt')
    parser.add_argument('--det_image_resize', default=300, type=int)
    parser.add_argument('--det_model_weights',
                        default='/home/yaok/software/caffe_ssd/models/VGGNet/fbox/SSD_300x300/'
                        'VGG_fbox_SSD_300x300_iter_10000.caffemodel') # 6000 34000

    parser.add_argument('--seg_model_def',
                        default='/home/yaok/software/caffe_ssd/models/VGGNet/fbox/vocfcn8s/deploy.prototxt')
    parser.add_argument('--seg_image_resize', default=500, type=int)
    parser.add_argument('--seg_model_weights',
                        default='/home/yaok/software/caffe_ssd/models/VGGNet/fbox/vocfcn8s/'
                        'voc-fcn8s-box-train_iter_64000.caffemodel') # 6000 34000
